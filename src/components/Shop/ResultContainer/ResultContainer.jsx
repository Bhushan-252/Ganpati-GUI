...
function ResultContainer() {
    ...
    const sidebarClass = `flex justify-between flex-col items-center lg:items-start 
    overflow-hidden transition-transform max-md:h-[100dvh] max-md:w-full 
    lg:w-fit bg-white border-r-1 border-gray-300 max-md:fixed top-0 z-10 
    lg:pr-[5%] lg:pl-2 ${filterOn ? "max-sm:translate-x-[-30%]" : "max-sm:translate-x-[-100%]"}`;
            ...
            <div className="max-md:relative lg:flex my-0.5 bg-[#fff] z-10 dark:bg-black">
                ...
                <div className={sidebarClass}>
                    ...
                    <div className="max-md:self-center max-md:ml-[19%] p-2">
                        ...
                        <FilterCheckbox
                            value={0}
                            title="Shadu Soil Idol"
                            filterStore={filterStore}
                            setFilterStore={setFilterStore}
                        />
                        <FilterCheckbox
                            value={1}
                            title="Paper Clay Idol"
                            filterStore={filterStore}
                            setFilterStore={setFilterStore}
                        />

                        <h3 className="text-xl font-semibold border-b-2 w-[50%] mt-3">
                            Sizes
                        </h3>
                        {Inches.map((value, index) => (
                            <FilterCheckbox
                                key={index}
                                title={value}
                                value={value}
                    ...
                    <button
                        ...
                        className="cursor-pointer lg:hidden active:scale-95 outline-0
              max-md:ml-20 max-md:w-1/2 relative self-center inline-flex
              items-center justify-center p-0.5 mb-2 me-2 overflow-hidden
              ...
              group-hover:from-purple-600 group-hover:to-blue-500 hover:text-white"
                    >
            <span className="relative px-5 py-2.5 transition-all w-full ease-in duration-75 bg-white rounded-md group-hover:bg-transparent">
              Apply
            </span>
                    </button>
                </div>
                ...
                {/* Main Content */}
                <div
                    className="lg:flex lg:flex-col lg:w-full p-4"
                    onClick={() => filterOn && setFilterOn(false)}
                >
                    {/* Sort Dropdown */}
                    <div className="flex lg:flex-row-reverse text-weback items-center justify-between border-b-2 border-wefront">
                        <div
                            className="flex items-center font-semibold outline-0 lg:hidden bg-gray-50
                border border-gray-300 text-gray-900 mb-2 text-sm rounded-lg
                focus:ring-blue-500 focus:border-blue-500 p-2.5"
                        ...
                        <select
                            ...
                            className="max-sm:w-32 md:w-40 lg:w-56 font-semibold m-2 outline-0 self-end
                bg-gray-50 border border-gray-300 text-gray-900 mb-4 text-sm
                rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5"
                    ...
                    {/* Product Grid */}
                    {(isLoading || error) ? (
                        <div className="grid gap-2 max-sm:grid-cols-2 lg:grid-cols-4 p-2">
                            {[...Array(4)].map((_, i) => (
                                <div
                                    key={i}
                                    role="status"
